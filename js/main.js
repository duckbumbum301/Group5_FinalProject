...
// Removed unused import: products, filterProducts from './products.js'
...
let filters = {
  q: '',
  cat: 'all',
  sub: null,           // <— NEW: subcategory filter
  sort: 'pop',
  priceMax: 250000,
  favOnly: false
};
...
function applyFiltersAndRender() {
  ...
  if (filters.sub) {                      // <— NEW: filter theo sub
    items = items.filter(p => p.sub === filters.sub);
  }
  ...
  if (filters.favOnly) {
    items = items.filter(p => getFavorites().includes(p.id));
  }
...
function handleMegaMenuLinkClick(e) {
  ...
  filters.sub = subcategory || null;      // <— NEW: set sub
...
function setupListeners() {
  ...
  // Khi đổi danh mục trong select → reset sub để không “kẹt” tiểu mục cũ
  catFilter.addEventListener('change', e => {
    filters.cat = e.target.value;
    filters.sub = null;                         // <— NEW
    applyFiltersAndRender();
  });
  ...
  // Delegation cho link trong mega menu (1 nơi duy nhất)
  productsMegaMenu.addEventListener('click', handleMegaMenuLinkClick);

  // Close khi click ngoài
  document.addEventListener('click', (e) => {
    if (!navItem.contains(e.target)) closeMegaMenu();
  });

  // Grid & Cart delegation
  ...
  // ⚠️ ĐÃ GỠ KHỐI forEach gắn click .mega-menu__link trùng lặp (tránh double-binding)
}
...
// Mini Admin API: thêm/sửa nhanh cho demo
window.Admin = {
  add(product) {
    PRODUCTS.push({
