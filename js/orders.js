...
function computeDerivedTracking(order) {
  const base =
    order.tracking && Array.isArray(order.tracking)
  ...
  return `<div class="timeline">${steps
    .map((s) => {
      ...
      return `<div class="step ${
        ...
        time ? `<div class=\"time muted\">${time}</div>` : ""
      }</div>\n      </div>`;
    })
    .join("")}</div>`;
}
  ...
  if (!data || !data.length) {
    ...
    const html = await Promise.all(
      data
        .slice()
        .sort((a, b) => {
          // 👇 Sắp xếp theo thời gian tạo đơn: mới nhất lên đầu
          const timeA = new Date(a.created_at || 0).getTime();
          const timeB = new Date(b.created_at || 0).getTime();
          return timeB - timeA; // Đơn mới hơn (timeB lớn hơn) lên trước
        })
        .map(async (o) => {
          const entries = Object.entries(o.items || {}).filter(
            ([, q]) => q > 0
          );
          const ids = entries.map(([pid]) => pid);
          const products = await Promise.all(
            ids.map((id) => apiGetProductById(id))
          );
          const map = {};
...
export function openOrderConfirmModal(orderId) {
  ...
  (async () => {
    ...
    const ord =
      (list || []).find((x) => x.id === orderId) ||
      (list || [])[list.length - 1];
    if (!ord) {
      body.innerHTML = '<p class="alert">Không tìm thấy đơn hàng.</p>';
      return;
    }
    ...
    body.innerHTML = `
      ...
      <div class="oc-pay">
        ...
        <div class="oc-row"><div>Tiền hàng</div><div>${money(
          ord.subtotal || 0
        )}</div></div>
        <div class="oc-row"><div>Phí giao hàng, phụ phí</div><div>${money(
          ord.shipping_fee || 0
        )}</div></div>
        ...
        <div class="oc-row"><div>Đã thanh toán</div><div>${money(
          paid
        )}</div></div>
    ...
    document.dispatchEvent(
      new CustomEvent("vvv:order_details_rendered", {
  ...
  if (!m.hasAttribute("data-bound")) {
    document
      .getElementById("ocOverlay")
      ?.addEventListener("click", () => closeOrderConfirmModal());
    document
      .getElementById("ocCloseBtn")
      ?.addEventListener("click", () => closeOrderConfirmModal());
...
export function openOrderSuccessModal(orderId) {
  ...
  const goShopping = () => {
    const target = location.pathname.endsWith("/index.html")
      ? "#catalog"
      : "../html/index.html#catalog";
  ...
  const html = await Promise.all(
    data
      ...
      .map(async (o) => {
        ...
        const map = {};
        for (const p of products) if (p && p.id) map[p.id] = p;
        const thumbsHtml = entries
          .slice(0, 5)
          .map(([pid]) => {
            const p = map[pid];
            if (p?.image)
              return `<img src="${p.image}" alt="${p.name}" class="orders-card__thumb">`;
            return `<div class="orders-card__thumb">${p?.emoji || "🧺"}</div>`;
          })
          .join("");
        const moreCount = Math.max(0, entries.length - 5);
        const moreHtml = moreCount
          ? `<div class="orders-card__thumb more">+${moreCount}</div>`
          : "";
        const paidText =
          o.payment_status === "paid"
            ? `<div class="muted">Đã thanh toán: ${money(o.total || 0)}</div>`
            : "";
        ...
        const hasReturnedOrCancelled = steps.some(
          (s) => s.code === "returned" || s.code === "cancelled"
        );
          ...
          <div class="orders-card" data-id="${o.id}">
            <div class="orders-card__head">
              ...
              <div class="orders-card__actions">
                <a href="#" data-action="detail" data-id="${
                  o.id
                }">Xem chi tiết ›</a>
              </div>
            </div>
            <div class="muted">Giao lúc: ${o.slot || "-"} · Mua lúc: ${fmtDate(
          o.created_at
        )}</div>
            <div class="orders-card__thumbs">${thumbsHtml}${moreHtml}</div>
            <div class="orders-card__foot">
              <div>
                <div><span class="muted">Tổng đơn hàng:</span> ${money(
                  o.total || 0
                )}</div>
                ${paidText}
              </div>
              <div class="orders-card__ops">
                <button class="btn vvv-review-order-btn" data-action="review-order" data-id="${
                  o.id
                }">Đánh giá</button>
                <button class="btn btn--outline" data-action="reorder" data-id="${
                  o.id
                }">Mua lại đơn</button>
                <button class="btn btn--danger" data-action="return" data-id="${
                  o.id
                }" ${
          returnable
            ? ""
            : 'disabled title="Chỉ trả hàng sau khi giao thành công"'
        }>Trả hàng</button>
              </div>
            </div>
          </div>`;
