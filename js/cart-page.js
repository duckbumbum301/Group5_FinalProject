...
import { loadCart, getCart, updateCartQuantity, removeFromCart, clearCart, addToCart } from './cart.js';
...
function loadOpenPanels(){
  try {
    const arr = JSON.parse(localStorage.getItem('vvv_cart_similar_open') || '[]');
    openSimilarPanels = new Set((arr || []).map(String));
  } catch { openSimilarPanels = new Set(); }
}
function saveOpenPanels(){
  try {
    localStorage.setItem('vvv_cart_similar_open', JSON.stringify(Array.from(openSimilarPanels)));
  } catch {}
}
...
function renderPmReviews(productId) {
  ...
  const avg = Math.round(data.reduce((s, r) => s + (Number(r.rating) || 0), 0) / data.length);
  const avgStars = renderStars(avg);

  const renderItems = (arr) => arr.map(r => {
    const short = sanitizeInline(String(r.comment || '').trim().slice(0, 160));
  ...
  cont.innerHTML = `
    ...
    ${hasMore ? `<button type="button" class="pm-reviews__show-more" id="pmShowMoreBtn">Xem thêm đánh giá (${data.length - 3})</button>` : ''}
  `;
...
async function openProductModal(productId) {
  ...
  let ratingMap = {}; try { ratingMap = JSON.parse(localStorage.getItem('vvv_rating') || '{}'); } catch {}
  if (!ratingMap[p.id]) {
  ...
  if (descEl) descEl.textContent = p.desc || 'Sản phẩm tươi ngon, giao nhanh trong ngày. (Mô tả demo)';
  if (thumbEl) {
    thumbEl.innerHTML = p.image ? `<img src="${p.image}" alt="${p.name}" style="width:160px;height:160px;object-fit:cover;border-radius:12px;" />` : (p.emoji || '🛒');
  }
...
async function toggleSimilarBelow(pid, lineEl, mode='toggle'){
  ...
  let sims = all.filter(sp => sp.id !== pid && sp.cat === p.cat && sp.sub === p.sub);
  if (sims.length < 4) {
    const more = all.filter(sp => sp.id !== pid && sp.cat === p.cat && sp.sub !== p.sub);
    sims = sims.concat(more);
  }
  sims = sims.slice(0, 6);
  const gridHtml = sims.map(sp => {
    const eff = getFlashEffectivePrice ? getFlashEffectivePrice(sp) : sp.price;
    const img = sp.image || '../images/brand/LogoVVV.png';
    // Average star rating: from reviews if available, fallback to local rating map
    let avg = 0;
    try {
      const reviews = (window.VVVReviews?.Local?.getAll?.() || []).filter(r => String(r.productId) === String(sp.id));
      if (reviews.length) avg = Math.round(reviews.reduce((s, r) => s + (Number(r.rating) || 0), 0) / reviews.length);
    } catch {}
    if (!avg) {
      let ratingMap = {}; try { ratingMap = JSON.parse(localStorage.getItem('vvv_rating') || '{}'); } catch {}
      if (!ratingMap[sp.id]) {
        ratingMap[sp.id] = Math.max(1, Math.min(5, Math.floor(Math.random() * 5) + 1));
        try { localStorage.setItem('vvv_rating', JSON.stringify(ratingMap)); } catch {}
      }
    ...
    return `
      <div class="sim-card" data-id="${sp.id}">
        <div class="sim-thumb"><img src="${img}" alt="${sanitizeInline(sp.name)}" onerror="this.onerror=null; this.src='../images/brand/LogoVVV.png';" /></div>
...
async function renderCartPage(){
  ...
  const lines = entries.map(([pid, qty]) => {
    ...
    return `
      <div class="cart-line" data-id="${p.id}" style="display:grid; grid-template-columns: 28px 64px 180px 120px 110px 120px 110px minmax(180px, 1fr); align-items:center; gap:12px; padding:12px 0; border-bottom:1px solid #eee;">
        ...
        <div class="thumb"><img src="${img}" alt="${p.name}" style="width:64px; height:64px; object-fit:cover; border-radius:6px;" onerror="this.onerror=null; this.src='../images/brand/LogoVVV.png';" /></div>
        ...
        <div class="line-total" style="text-align:right; font-weight:600; padding-right:8px; white-space:nowrap; pointer-events:none;">${money(effPrice * qty)}</div>
  ...
  selAllEl && (selAllEl.checked = entries.every(([pid]) => selectedIds.has(String(pid))));
}
...
function bindCartPage(){
  ...
  body?.addEventListener('click', (e) => {
    ...
    if (simCard && simCard instanceof HTMLElement) {
      ...
      if (sid) { openProductModal(sid); return; }
    }
    const line = rootEl ? rootEl.closest('.cart-line') : null;
    const pid = line?.dataset?.id;
    if (!pid) return;
    // Open product detail when clicking image or name
    if (rootEl.closest('.thumb') || rootEl.closest('.cart-name')) { openProductModal(pid); return; }
  ...
  body?.addEventListener('change', (e) => {
    ...
    if (t.dataset.action === 'select') {
      ...
      const pid = line?.dataset?.id;
      if (!pid) return;
      if (t.checked) selectedIds.add(String(pid)); else selectedIds.delete(String(pid));
      renderCartPage();
    }
  });

  btnClear?.addEventListener('click', () => { clearCart(); openSimilarPanels.clear(); saveOpenPanels(); });
  ...
  selAllEl?.addEventListener('change', () => {
    ...
    const ids = Object.entries(getCart()).filter(([, q]) => q > 0).map(([pid]) => String(pid));
